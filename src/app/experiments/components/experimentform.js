        ...
        <>
            <form className="flex flex-col gap-4 max-w-md mx-auto" onSubmit={handleSubmit}>
                ...
                <div className="flex items-center gap-2">
                    <input id="useBuilder" type="checkbox" checked={useBuilder} onChange={(e) => setUseBuilder(e.target.checked)} />
                    <label htmlFor="useBuilder" className="text-sm">Use Payload Builder</label>
                </div>
                ...
                {useBuilder && (
                    <div className="p-3 border rounded bg-gray-50">
                        <div className="flex flex-col mb-2">
                            ...
                            <input value={builder.name} onChange={(e) => setBuilder(b => ({ ...b, name: e.target.value }))} className="p-2 border rounded" />
                        </div>
                        <div className="flex gap-2 mb-2">
                            <div className="flex-1">
                                <label className="mb-1 text-sm font-medium">Bay Width</label>
                                <input type="number" value={builder.bay_width} onChange={(e) => setBuilder(b => ({ ...b, bay_width: Number(e.target.value) }))} className="p-2 border rounded w-full" />
                            </div>
                            <div className="flex-1">
                                <label className="mb-1 text-sm font-medium">Bay Height</label>
                                <input type="number" value={builder.bay_height} onChange={(e) => setBuilder(b => ({ ...b, bay_height: Number(e.target.value) }))} className="p-2 border rounded w-full" />
                            </div>
                        </div>
                        ...
                        <div>
                            <div className="flex items-center justify-between mb-2">
                                ...
                                <button type="button" className="text-sm text-blue-600" onClick={addBuilderItem}>Add Item</button>
                            </div>
                            <div className="space-y-2">
                                {(builder.items||[]).map((it, idx) => (
                                    <div key={idx} className="flex gap-2 items-center">
                                        <select value={it.module_id ?? ''} onChange={(e) => updateBuilderItem(idx, { module_id: e.target.value ? Number(e.target.value) : null })} className="p-2 border rounded">
                                            <option value="">Select module</option>
                                            {modules.map(m => (
                                                <option key={m.id} value={m.id}>{m.name}</option>
                                            ))}
                                        </select>
                                        <input type="number" value={it.x} onChange={(e) => updateBuilderItem(idx, { x: Number(e.target.value) })} className="p-2 border rounded w-16" />
                                        <input type="number" value={it.y} onChange={(e) => updateBuilderItem(idx, { y: Number(e.target.value) })} className="p-2 border rounded w-16" />
                                        <input type="text" value={it.label} onChange={(e) => updateBuilderItem(idx, { label: e.target.value })} className="p-2 border rounded flex-1" placeholder="label" />
                                        <button type="button" className="text-red-500" onClick={() => removeBuilderItem(idx)}>Remove</button>
                                    </div>
                                ))}
                            </div>
                        </div>
            ...
            {success && (
                <div className="mt-6 max-w-md mx-auto p-4 border border-green-200 rounded bg-green-50">
                    ...
                    <div className="text-sm text-green-700 mt-2">ID: {success.experiment_id}</div>
                    <div className="text-sm text-green-700">Name: {success.experiment?.name}</div>
                    <div className="text-sm text-green-700">Description: {success.experiment?.description}</div>
                    <div className="text-sm text-green-700">Status: {success.experiment?.status}</div>
                    <div className="text-sm text-green-700">Files saved: {success.files_saved ?? 0}</div>
                </div>
            )}
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center">
                    ...
                    <div className="relative bg-white rounded shadow-lg max-w-lg w-full p-6 z-10">
                        ...
                        <div className="mb-4 text-sm">
                            <div><strong>ID:</strong> {success.experiment_id}</div>
                            <div><strong>Name:</strong> {success.experiment?.name}</div>
                            <div><strong>Description:</strong> {success.experiment?.description}</div>
                            <div><strong>Status:</strong> {success.experiment?.status}</div>
                            <div><strong>Files saved:</strong> {success.files_saved ?? 0}</div>
                        </div>
                        <div className="flex justify-end">
                            <button className="bg-blue-600 text-white px-4 py-2 rounded" onClick={() => setShowModal(false)}>Close</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            <div className="mt-8 max-w-2xl mx-auto">
                ...
                {submissions.length === 0 && <div className="text-sm text-neutral-500">No submissions yet.</div>}
                <ul className="space-y-4">
                    {submissions.map((s) => (
                        <li key={s.id} className="p-3 border rounded">
                            <div className="font-medium">{s.name} <span className="text-xs text-neutral-500">(ID: {s.id})</span></div>
                            <div className="text-sm text-neutral-700">{s.description}</div>
                            <div className="text-xs text-neutral-500">Status: {s.status} • Files: {s.files?.length ?? 0}</div>
                        </li>
                    ))}
                </ul>
            </div>
